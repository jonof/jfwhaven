...
{
	int i;
	short angvel=loc.angvel, svel=loc.svel, vel=loc.vel, horizvel=0;
	int mousx, mousy, bstatus;

	if(keystatus[keys[KEYSTRAFE]] == 0) {
		if(keystatus[keys[KEYLEFT]] || keystatus[RLEFT]) angvel-=16*TICSPERFRAME;
		if(keystatus[keys[KEYRIGHT]] || keystatus[RRIGHT]) angvel+=16*TICSPERFRAME;
	}
	else {
		if(keystatus[keys[KEYLEFT]] || keystatus[RLEFT]) svel+=8*TICSPERFRAME;
		if(keystatus[keys[KEYRIGHT]] || keystatus[RRIGHT]) svel-=8*TICSPERFRAME;
	}

	if (keystatus[keys[KEYSTFL]]) svel+=8*TICSPERFRAME;
	if (keystatus[keys[KEYSTFR]]) svel-=8*TICSPERFRAME;

	if(keystatus[keys[KEYFWD]] || keystatus[RUP]) vel+=8*TICSPERFRAME;
	if(keystatus[keys[KEYBACK]] || keystatus[RDN]) vel-=8*TICSPERFRAME;

	if(keystatus[keys[KEYLKDN]]) horizvel = -(TICSPERFRAME<<2);
	if(keystatus[keys[KEYLKUP]]) horizvel = (TICSPERFRAME<<2);

	if( option[3] & 1 ) {
		getmousevalues(&mousx,&mousy,&bstatus);
		if(keystatus[keys[KEYSTRAFE]]) svel-=(mousx*mousxspeed);
		else angvel+=(mousx*mousxspeed);                                 // Les 07/24/95
		...
		if (mouselook) horizvel-=(mousy>>1);
		else vel-=(mousy*mousyspeed);                            // Les 07/24/95
		for (i=0 ; i < 2 ; i++) {
	...
	if (jstickenabled) {
		...
		if (joynumaxes >= 6) {
			keystatus[keys[KEYFIRE]] |= (joyaxis[5] > 0);	// Rtrigger shoot
		}
	...
	horizvel = max(-(YDIM>>1),min((YDIM>>1),horizvel));
	...
	for (i=KEYWEAP1; i<=KEYWEAP10; i++)
		if (keystatus[keys[i]]) loc.keys.weapon = 1+i-KEYWEAP1, keystatus[keys[i]]=0;
	loc.keys.spell = 0;
	for (i=KEYSPELL1; i<=KEYSPELL8; i++)
		if (keystatus[keys[i]]) loc.keys.spell = 1+i-KEYSPELL1, keystatus[keys[i]]=0;
}
...
void processinput(struct player *plr) {
	...
	if(plr->dimension == 2) {
		if(keystatus[keys[KEYZOOMO]] > 0)                     // Les 07/24/95
			if(plr->zoom > 48)
				plr->zoom-=(synctics/TICSPERFRAME)*(plr->zoom>>4);
		...
		if (keystatus[keys[KEYZOOMO]] != 0 && plr->screensize > 64) { // Les 07/24/95
			plr->screensize-=(synctics/TICSPERFRAME)*8;
			didchange = 1;
		}
